...
LEFT JOIN dbo.Liv3Uff l3 ON l3.ID_Uff3=i.id_uff3liv WHERE pe.id_pe=@Id";
        ...
        {
            const string sql="INSERT dbo.PE_StoricoModifiche(ID_Pe,Campo_Variato,Data_Modifica,Utente_Modificatore,Valore_Vecchio,Valore_Nuovo) VALUES(@Id,@Campo,GETDATE(),@Utente,@Prima,@Dopo)";
            using(var command=new SqlCommand(sql,c,t))
            {
                command.Parameters.Add("@Id",SqlDbType.Int).Value=id;
                command.Parameters.Add("@Campo",SqlDbType.NVarChar,100).Value=campo;
                command.Parameters.Add("@Utente",SqlDbType.NVarChar,100).Value=(object)utente??DBNull.Value;
                // Lo storico rappresenta i valori assenti con una stringa vuota, non con NULL.
                command.Parameters.Add("@Prima",SqlDbType.NVarChar,-1).Value=prima??string.Empty;
                command.Parameters.Add("@Dopo",SqlDbType.NVarChar,-1).Value=dopo??string.Empty;
                command.ExecuteNonQuery();
            }
        }
